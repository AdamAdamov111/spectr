...
export function MapScreen() {
  ...
  const riskMarkers = useMemo(() => { const w = world(); const out = markers.slice(); for (const n of w.byType.get('PumpStation') || []) { let risk = 0; for (const e of neighbors(session, n.id, { types: ['has_equipment'] }).slice(0, 40)) for (const mo of neighbors(session, e.other.id, { types: ['maintains'] }).slice(0, 3)) for (const p of neighbors(session, mo.other.id, { types: ['performed_by'] })) risk = Math.max(risk, (p.other.props.risk_score as number) || 0); const m = out.find(x => x.id === n.id); if (m) m.risk = risk } return out }, [markers])
    ...
    <div className="screen map-screen">
      ...
      <div className="screen-b">
        ...
        <aside className="graph-side">
          <Panel title={<span className="row"><Layers size={12} /> Слои</span>} dense><div className="col" style={{ padding: 8, gap: 2 }}>{([['fields', 'Месторождения'], ['pipelines', 'Трубопроводы (поток ∝ расходу)'], ['wells', 'Скважины и кусты'], ['nps', 'НПС'], ['tanks', 'Резервуары (уровень)'], ['vehicles', 'Техника (ГЛОНАСС)'], ['anomalies', 'Аномалии'], ['incidents', 'Инциденты'], ['heat', 'Тепловая карта рисков контрагентов']] as [keyof MapLayers, string][]).map(([k, l]) => <label key={k} className="check"><input type="checkbox" checked={layers[k]} onChange={e => setLayers(s => ({ ...s, [k]: e.target.checked }))} />{l}</label>)}</div></Panel>
          <Panel title="Активы" dense><div className="col" style={{ padding: 8, gap: 4 }}>{[...(w.byType.get('PumpStation') || []), ...(w.byType.get('Field') || [])].map(o => <ObjectChip key={o.id} o={o} onOpen={x => { setFly({ x: x.geo![0], y: x.geo![1], zoom: x.type === 'Field' ? 2.2 : 3.2, key: Date.now() }); openObject(x.id) }} />)}</div></Panel>
          <Panel title="Табличная альтернатива" dense><div style={{ padding: 8 }} className="dim">Список активов доступен на экране Поиск (тип: Скважина, НПС, Резервуар) — для экранных дикторов (AC-04).</div></Panel>
        </aside>
      </div>
    </div>
  )
