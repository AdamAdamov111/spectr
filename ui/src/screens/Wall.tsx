...
export function Wall() {
  ...
  useEffect(() => subscribeEvents(() => { const e = world().events[0]; if (e && (e.kind === 'incident' || (e.kind === 'anomaly' && e.text.includes('НПС-2')))) { setIncident(e.text); setTimeout(() => setIncident(null), 20000) } }), [])
  ...
  return (
    <div className="wall">
      <div className="wall-map"><MapCanvas markers={markers} pipelines={w.pipelines} layers={{ ...DEFAULT_LAYERS, vehicles: true }} onSelect={() => {}} ambient theme="dark" wall />
        <div className="wall-corner"><span className="mono">{pad(clock.getHours())}:{pad(clock.getMinutes())}:{pad(clock.getSeconds())}</span><span className="dim">Северная нефть · СЦ · онтология v{ONTOLOGY_VERSION} · цель situation_wall · INTERNAL</span></div>
      </div>
        ...
        {incident ? <div className="wall-incident fade-in"><div className="wall-incident-h">ИНЦИДЕНТ</div><div className="wall-incident-t">{incident}</div><div className="dim">ротация панелей остановлена</div></div> : (
          <div className="wall-panel fade-in" key={panel}>
            {panel === 0 && <><div className="wall-h">Ключевые показатели</div><div className="wall-kpis">
              <div className="wall-kpi"><span>Добыча</span><b><CountUp value={k.debit} /> <small>т/сут</small></b></div>
              <div className="wall-kpi"><span>Транспорт</span><b><CountUp value={k.flow} /> <small>м³/ч</small></b></div>
              <div className="wall-kpi danger"><span>Инциденты</span><b><CountUp value={k.incidents} /></b></div>
              <div className="wall-kpi warn"><span>Аномалии</span><b><CountUp value={k.anomalies} /></b></div>
              <div className="wall-kpi"><span>ТОиР открыто</span><b><CountUp value={k.openOrders} /></b></div>
              <div className="wall-kpi"><span>Скважин в работе</span><b><CountUp value={k.wells} /> <small>/ {k.wellsTotal}</small></b></div>
            </div></>}
